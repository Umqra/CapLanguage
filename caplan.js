...
function printError(lineNumber, error){
	ListErrors.push("Error at line " + lineNumber + ": " + error);
...
function ReadClass() {
	this.buffer = document.getElementsByName("input")[0].value;
	this.bufferPtr = 0;
...
function preprocessCode(codeLines) {
	Read = new ReadClass();
	UsedMemory = new Array(MemorySize);
	LabelPosition = new Object();
	VariableAddress = new Object();
	ListErrors = new Array();
